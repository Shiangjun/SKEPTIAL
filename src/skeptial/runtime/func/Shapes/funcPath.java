...
public class funcPath extends funcShape {
	private List<funcPoint>  pts = new ArrayList<funcPoint>();
	...
	public funcPath(int _LineColor, float Linewidth){
		this();
		LineColor  = _LineColor;
		StrokeWidth = Linewidth;
	}
	
	public funcPoint GetTrasformedPoint(funcPoint pt, float ScreenWidth, float ScreenHeight, Boolean withNormalization){
		//1-translate
		//2-rotate
		//3-scale
	...
	public funcPoint getPoint (int i){
		return pts.get(i);
	}
	public void setPoint(int i, funcPoint pt){
		pts.get(i).x = pt.x;
		pts.get(i).y = pt.y;
	}

	public void AddPoint(float x ,float y){
		pts.add(new funcPoint(x,y));
	}
	@Override
		...
		//with smoothing 
			...
			if(PointsCount() > 1){
		        for(int i = PointsCount() - 2; i <PointsCount(); i++){
		            if(i >= 0){
		                funcPoint point = pts.get(i);

		                if(i == 0){
		                	funcPoint next = pts.get(i + 1);
		                    point.dx = ((next.x - point.x) / 3);
		                    point.dy = ((next.y - point.y) / 3);
		                }
		                else if(i == PointsCount() - 1){
		                	funcPoint prev = pts.get(i - 1);
